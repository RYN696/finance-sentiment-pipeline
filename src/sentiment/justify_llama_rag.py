...
from config import SENTIMENTS_DIR, JUSTIFICATIONS_DIR, PROCESSED_DIR, MODEL_LLAMA3
...
7. Do not force information from the evidence into the justification.
8. Do not invent or infer facts that are not supported by the main article or relevant evidence.
9. Focus on concrete financial factors, events, results, trends, or indicators that explain the sentiment.
10. Do not simply repeat the sentiment label.
...
def run(input_path=None, output_path=None):
    input_path = input_path or (SENTIMENTS_DIR / "sentiment_finbert.json")
    output_path = output_path or (JUSTIFICATIONS_DIR / "llama3.1_justifications_rag.json")
    ...
    resultats = []
    print(f"Génération de justifications RAG pour {len(data)} articles (Llama 3.1)...")

    for i, article in enumerate(data):
        texte = texte_par_id.get(article["article_id"], article["title"])
        reponse, passages = generer_justification_rag(texte, article["finbert_label"], article["entreprise_cible"])
        ...
        resultats.append({
            ...
            "llama_justification_rag": reponse.replace("Justification:", "").strip(),
