...
5. You do NOT need to use every evidence passage.
6. Ignore evidence that is irrelevant, redundant, ambiguous, outdated, or contradictory to the main article.
7. Do not invent or infer facts that are not supported by the main article or relevant evidence.
8. Focus on concrete financial factors, events, results, trends, or indicators that explain the sentiment.
9. Do not simply repeat the sentiment label.
10. Respond in plain text only, no markdown, no asterisks.
...
def construire_bloc_evidence(passages):
    ...
    return "\n".join(f"- Evidence {i+1}: {p['chunk_text']}" for i, p in enumerate(passages))
...
def generer_justification_rag(texte, label, entreprise_cible):
    ...
    return nettoyer_markdown(response["response"]), passages

def run(input_path=None, output_path=None):
    input_path = input_path or (SENTIMENTS_DIR / "sentiment_financellama.json")
    output_path = output_path or (JUSTIFICATIONS_DIR / "financellama_justifications_rag.json")
    ...
    resultats = []
    print(f"Génération de justifications RAG pour {len(data)} articles (Finance-Llama)...")

    for i, article in enumerate(data):
        texte = texte_par_id.get(article["article_id"], article["title"])
        reponse, passages = generer_justification_rag(texte, article["financellama_label"], article["entreprise_cible"])
        ...
        resultats.append({
            ...
            "financellama_label": article["financellama_label"],
            "financellama_justification_rag": reponse.replace("Justification:", "").strip(),
