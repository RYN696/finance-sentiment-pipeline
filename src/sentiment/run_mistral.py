...
from config import PROCESSED_DIR, RESULTATS_MISTRAL, SENTIMENTS_DIR, MODEL_MISTRAL
...
def run(output_path=None):
    ...
    for i, article in enumerate(articles):
        ...
        print(f"  [{i + 1}/{len(articles)}] {article['entreprise_cible']} ({article['source_name']}) -> {resultats[-1]['mistral_label']}")
